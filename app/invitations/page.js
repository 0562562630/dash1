...
export default function InvitationsPage() {
  ...
  if (saved.length > 0) {
    setInvitations((prev) => {
      ...
      // إزالة التكرار بناءً على العنوان والتاريخ
      const uniqueInvitations = combined.filter(
        (item, index, self) =>
    ...
    setReminded((prev) => Array(saved.length + prev.length).fill(false));
  }
}, []);


  const handleDelete = (index) => {
  const deletedItem = invitations[index];
  ...
  // حذف من localStorage إذا كانت من الدعوات المضافة يدويًا
  const stored = JSON.parse(localStorage.getItem('newInvitations') || '[]');
  const updatedStored = stored.filter((inv) => inv.title !== deletedItem.title);
  localStorage.setItem('newInvitations', JSON.stringify(updatedStored));
};
