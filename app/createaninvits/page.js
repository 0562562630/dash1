...
const CreateInvitation = () => {
  ...
  const handleSave = () => {
    const newDate = selectedDate ? selectedDate.toLocaleDateString('en-GB') : '';
    const existing = JSON.parse(localStorage.getItem('newInvitations') || '[]');

    // ✅ التحقق من تكرار الدعوة بنفس العنوان والتاريخ
    const isDuplicate = existing.some((inv) => inv.title === title && inv.date === newDate);
    if (isDuplicate) {
      alert('⚠️ هذه الدعوة موجودة بالفعل بنفس العنوان والتاريخ.');
      return;
    }
    ...
    const saveToLocalStorage = (invitationWithImage = null) => {
      const invitation = invitationWithImage || newInvitation;
      localStorage.setItem('newInvitations', JSON.stringify([invitation, ...existing]));
    ...
    if (imageFile) {
      ...
      reader.onloadend = () => {
        newInvitation.image = reader.result;
        saveToLocalStorage(newInvitation);
      };
